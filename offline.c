		...
		{	
			...
			{				
				...
				for(k=em[j].floor;k<=max_floor;k++)//Starting part
				{
					...
					if(em[j].floor == k && (q[k].nu != 0 && q[k].dn[0][1]==i) && (e[j].floor_handled == 0 || e[j].floor_handled == k) && em[j].current_time<=i)
					{
						int count=0;
			...
			else if(em[j].move_up==false && em[j].move_down==true)
			{
				...
				//			printf("\nEntered the b loop");
					...
					if(em[j].floor == k && (q[k].nd != 0 && q[k].dn[0][1]==i)  && (e[j].floor_handled == 0 || e[j].floor_handled == k) && em[j].current_time<=i)
					{
						int count=0;
						for(m=0;m<no_of_lift;m++)
						{
