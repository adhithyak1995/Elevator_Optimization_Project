...
/*
	Need to implement the stputt
	op in between the travel
*/
...
{	
	...
	int p_s;
	int stop;
	int stop_movement;//To control the movement of the lift in between	1 for stop inbetween 0 for not stopping
	int st;
};
...
void main()
{
	...
	sch=fopen("C:\\Users\\leoad\\Downloads\\schedule_offline.txt","w");
	s_e=fopen("C:\\Users\\leoad\\Downloads\\s_e_offline.txt","w");
	s_p=fopen("C:\\Users\\leoad\\Downloads\\s_p_offline.txt","w");	
	...
	struct elevators e[no_of_lift];
		...
		//To find the average and maximal time to reach destination
	int time_decide[n_p];
	int decide = 0;
...
for(i=0;i<no_of_lift;i++)
	...
	for(i=1;i<=86400 || (endcheck == 0);i++)
	{
		...
		{			
			...
			{				
				...
				if(e[j].npe == 0)
				{
					for(f=1;f<=max_floor;f++)
					{
						...
						if(stop_counter==0)
						{
							em[j].stop_movement = 1;
							
							//printf("\nTime is %d Trying to restrict the movement of the elevator",i);
						}
						else
						{
							em[j].stop_movement = 0;
						}
				...
				for(k=em[j].floor;k<=max_floor;k++)//Starting part
				{
					...
					for(m=0;m<e[j].npe && em[j].current_time <= i && em[j].take_in==false ;m++)
					{
					...
					for(r=0;r<b;r++)
					{
						...
						time_decide[decide]=em[j].current_time - e[j].pe[g].arr_time;
						decide++;
						...
						//printf("\nNo of passengers in the elevator is %d",e[j].len);
							e[j].npe=e[j].npe-1;
							e[j].len=e[j].len-1;
							//Handling the stopping of the elevator				
											
					}	
					if(em[j].floor == k && em[j].es >= 2 && (q[k].nu != 0 && q[k].up[0][1]<=i) && (e[j].floor_handled == 0 || e[j].floor_handled == k) && em[j].current_time<=i && e[j].npe<10)
					{
					...
					{
					if(em[j].take_out == false && em[j].current_time <= i && em[j].take_in == false && em[j].stop_movement == 0)
					{
					...
					else if(em[j].take_out == false && em[j].current_time < i && em[j].take_in == false && em[j].stop_movement == 1)
					{
					//	printf("\nStopped elevator");
						fprintf(sch,"%d %d %d stop\n",i,j+1,em[j].floor);
						fprintf(s_e,"%d %d stop\n",i,em[j].floor);
					}
			...
			else if(em[j].move_up==false && em[j].move_down==true)
			{
				...
				//			printf("\nEntered the b loop");
					...
					if(em[j].floor == k && em[j].es >= 1 && (q[k].nd != 0 && q[k].dn[0][1]<=i)  && (e[j].floor_handled == 0 || e[j].floor_handled == k) && em[j].current_time<=i && e[j].npe<10)
					{
						//fprintf(sch,"Time is %d and passenger in elevator time is %d\n",i,q[k].dn[0][1]);
						int count=0;
						for(m=0;m<no_of_lift;m++)
		...
		if(stop_it == max_floor + no_of_lift)
		{
			...
			for(j=0;j<no_of_lift;j++)
			{
			//	printf("\nYou can stop the elevator %d now",j);
				em[j].stop = 1;
			}
		}
...
//		}
	...
	int serve=0;
	int max_serve=time_decide[0];
	for(i=0;i<decide;i++)
	{
		if(max_serve<time_decide[i])
		{
			max_serve=time_decide[i];
		}
		serve+=time_decide[i];
	}	
	fprintf(s_p,"\nAverage time to arrive to destination is %d\nMax time to arrive destinatin is %d",serve/n_p,max_serve);
