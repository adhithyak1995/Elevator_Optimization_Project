...
void main()
{
	...
	for(i=1;i<=100;i++)
	{
				...
				for(k=em[j].floor;k<=max_floor;k++)//Starting part
				{
					...
					for(r=0;r<b;r++)
					{
						if(r==0)
						{
							...
							g=del[r]-r*1;}
							fprintf(sch,"%d %d %d drop out %d\n",em[j].current_time+2*y,j+1,em[j].floor,e[j].pe[g].pid);
							fprintf(s_e,"%d %d drop out %d\n",em[j].current_time+2*y,em[j].floor,e[j].pe[g].pid);
							fprintf(s_p,"%d %d %d %d\n",e[j].pe[g].pid,j+1,e[j].pe[r].arr_time,em[j].current_time+y*2);
						y=1;
						for(s=g;s<e[j].npe;s++)
						{
						...
						em[j].current_time=em[j].current_time+2*y;
		...
		{
			printf("\nAt Elevator no. %d\n",j);
			printf("\nFlags\nFloor is %d\nMove up is %d\nMove down is %d\nTake in is %d\nTake out is %d",em[j].floor,em[j].move_up,em[j].move_down,em[j].take_in,em[j].take_out);
