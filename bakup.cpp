		...
		for(k=1;k<=m_f;k++)//k refers to the floor
		{
			...
			if(q[k].nu!=0)
			{
				...
				while(l<no_of_lift)
				{
					...
					d[l]=k-em[l].floor;
					//printf("\nThe distance between the floor and the floor at which the elevator %d is there is %d",l,abs(d[l]));
				...
				while(l<no_of_lift)
				{
					if(em[l].take_in==1 || em[l].take_out==1)
					{
						m=d[l];
					}
...
//Problem exists here have to decide on it
				printf("\n M is %d",m);
				while(l<no_of_lift)
				{
					if(em[l].take_in==1 || em[1].take_out==1)
					{			
				...
				if(k==em[x].floor && em[x].es==0 || em[x].mid==1)
				{
					em[x].take_in=1;
