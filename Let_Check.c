...
void move_elevator_up(struct elevator_movement *em, struct elevators *e, bool *first_floor)
{
	...
	else if(*first_floor == 1 && em->es==0 && em->check ==0 )
	{
		em->floor++;
...
void take_out(struct elevator_movement *em,struct elevators *e, int a[],int z)
{
	printf("\nEntered into the taking out part of the code\nNumber to take out is %d",a[0]);
	
		int i,j,c;
		printf("\nFirst element is %d",e->pe[0].pid);
		for(i=0;i<e->npe;i++)
			printf("\n%d",e->pe[i].pid);
		...
		for(i=0;i<z;i++)
		{
			for(j=0;j<e->npe;j++)
			{
				...
				if(a[i]==e->pe[j].pid)
				{
					printf("\nA is %d and PID is %d",a[i],e->pe[j].pid);
					//delete_element_pos(e,e->pe,j);
					printf("\nDeletion is always possible");
					printf("\n%d",e->pe[j].pid);
					
					if(e->npe<=1)
					{
						printf("\nDeleting one and only element of the structure");
						e->npe=0;
					}
					else
					{
						for(c=j;c<e->npe;c++)
						{
							e->pe[c].pid=e->pe[c+1].pid;
							e->pe[c].en_floor=e->pe[c+1].en_floor;
							e->pe[c].st_floor=e->pe[c+1].st_floor;
							e->pe[c].arr_time=e->pe[c+1].arr_time;
							e->pe[c].t=e->pe[c+1].t;
						}
						e->npe--;						
					}				
...
void main()
{
	...
	for(i=1;i<=15;i++)
	{
		...
		//Deep copy is made now
				...
				if(((k==em[x].floor && em[x].es==0) && (em[x].mid==1 || em[x].mid==0))&&em[x].take_out==false)
				{
		...
		//				printf("\nMove up to find the passenger and pick him up");
						if(em[x].first_floor==true)
						{
							printf("\nThe elevator number is %d and moving up call no 1",x);
		...
		//				printf("\nMove up to find the passenger and pick him up");
						if(em[x].first_floor==true)
						{
							printf("\nThe elevator number is %d and moving up call no 2",x);
							move_elevator_up(&em[x],&e[x],&em[x].first_floor);
							continue;
						}
		...
		//				printf("\nMove up to find the passenger and pick him up");
						if(em[x].first_floor==true)
						{
							printf("\nThe elevator number is %d and moving up call no 3",x);
							move_elevator_up(&em[x],&e[x],&em[x].first_floor);
							continue;
						}					
						}				
		...
		{
			//printf("\nTake out timer is %d and Current time is %d",em[a].take_out_timer,em[a].current_time);
			if(em[a].take_out_timer>em[a].current_time)
			{
				printf("\nSkipping movement");
				continue;
			}
			...
			if(em[a].start==1 && em[a].current_time==i)
			{
				...
				if(em[a].es!=0)
				{
					...
					{	
	//					printf("\nMoving up");
						printf("\nThe elevator number is %d and moving up call no 4",a);
				...
				else
				{
					...
					{
						//em[a].es=2;
						printf("\nThe elevator no is %d and moving up call no 5",em[a].floor);
						move_elevator_up(&em[a],&e[a],&em[a].first_floor);
					}
					else if(em[a].move_up==false && em[a].move_down==true && em[a].take_in==false && em[a].take_out==false) 
					{
