...
void main()
{
	...
	sch=fopen("C:\\Users\\leoad\\Downloads\\schedule_online.txt","w");
	s_e=fopen("C:\\Users\\leoad\\Downloads\\s_e_online.txt","w");
	s_p=fopen("C:\\Users\\leoad\\Downloads\\s_p_online.txt","w");	
			...
			{				
				...
				for(k=em[j].floor;k<=max_floor;k++)//Starting part
				{
					...
					if(em[j].take_out == false && em[j].current_time <= i && em[j].take_in == false)
					{
						if(em[j].es == 2)
						{
							fprintf(sch,"%d %d %d pause\n",i,j+1,em[j].floor);
						    fprintf(s_e,"%d %d pause\n",i,em[j].floor);
						}
						else
						{
